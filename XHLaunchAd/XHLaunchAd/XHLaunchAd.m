...
- (instancetype)init {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:XHLaunchAdDetailPageWillShowNotification
                                                          object:nil
                                                           queue:nil
                                                      usingBlock:^(NSNotification *_Nonnull note) {
                                                          weakSelf.detailPageShowing = YES;
                                                      }];
        [[NSNotificationCenter defaultCenter] addObserverForName:XHLaunchAdDetailPageShowFinishNotification
                                                          object:nil
                                                           queue:nil
                                                      usingBlock:^(NSNotification *_Nonnull note) {
...
- (void)setupImageAdForConfiguration:(XHLaunchImageAdConfiguration *)configuration {
    ...
    if (configuration.imageNameOrURLString.length && XHISURLString(configuration.imageNameOrURLString)) {
        ...
        if ([self.delegate respondsToSelector:@selector(xhLaunchAd:launchAdImageView:URL:)]) {
            ...
            XHWeakSelf
                [adImageView xh_setImageWithURL:[NSURL URLWithString:configuration.imageNameOrURLString]
                    placeholderImage:nil
                    GIFImageCycleOnce:configuration.GIFImageCycleOnce
                    options:configuration.imageOption
                    GIFImageCycleOnceFinish:^{
                        //GIF不循环,播放完成
...
- (void)addSkipButtonForConfiguration:(XHLaunchAdConfiguration *)configuration {
    ...
    if (configuration.customSkipView) {
        ...
        if (_skipButton == nil) {
            ...
            if (!CGRectIsEmpty(configuration.skipButtonFrame)) _skipButton.frame = configuration.skipButtonFrame;
...
#pragma clang diagnostic pop
    ...
    if (configuration.videoNameOrURLString.length && XHISURLString(configuration.videoNameOrURLString)) {
        ...
        if (pathURL) {
            ...
            XHWeakSelf
                [[XHLaunchAdDownloader sharedDownloader] downloadVideoWithURL:[NSURL URLWithString:configuration.videoNameOrURLString]
